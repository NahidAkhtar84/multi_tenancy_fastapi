"""test and status field added to service

Revision ID: 6cb307fb48a2
Revises: d0256ba061b2
Create Date: 2023-02-15 07:02:15.776141
...
revision = '6cb307fb48a2'
down_revision = 'd0256ba061b2'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.drop_index('ix_tenant_users_email', table_name='users', schema='gp')
    op.drop_index('ix_tenant_users_full_name', table_name='users', schema='gp')
    op.drop_index('ix_tenant_users_id', table_name='users', schema='gp')
    op.drop_table('users', schema='gp')
    op.drop_table('public_users', schema='shared')
    op.drop_table('public_companies', schema='shared')
    op.drop_index('ix_tenant_services_id', table_name='services', schema='gp')
    op.drop_index('ix_tenant_services_name', table_name='services', schema='gp')
    op.drop_table('services', schema='gp')
...
def downgrade() -> None:
    ...
    op.alter_column('tenants', 'id',
               ...
               schema='shared')
    op.create_table('services',
    sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('gp.services_id_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.Column('name', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.PrimaryKeyConstraint('id', name='services_pkey'),
    schema='gp'
    )
    op.create_index('ix_tenant_services_name', 'services', ['name'], unique=False, schema='gp')
    op.create_index('ix_tenant_services_id', 'services', ['id'], unique=False, schema='gp')
    op.create_table('public_companies',
    ...
    op.create_table('users',
    sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('gp.users_id_seq'::regclass)"), autoincrement=True, nullable=False),
    ...
    op.create_index('ix_tenant_users_id', 'users', ['id'], unique=False, schema='gp')
    op.create_index('ix_tenant_users_full_name', 'users', ['full_name'], unique=False, schema='gp')
    op.create_index('ix_tenant_users_email', 'users', ['email'], unique=False, schema='gp')
